    ...
    <el-dialog :title="title" :visible.sync="open" width="50%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="标题">
          <el-input v-model="form.title" maxlength="40" show-word-limit placeholder="请输入标题"/>
        </el-form-item>
...
<script>
import {listAbout, getAbout, delAbout, addAbout, updateAbout} from "@/api/lover/about";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        state: [
          {required: true, message: "清选择状态", trigger: "blur"}
        ],
        content: [
  ...
  methods: {
    updateState(item) {
      this.$modal.confirm('是否确认修改关于我们编号为"' + item.id + '"的数据项？').then(() => {
        this.loading = true;
        return updateAbout(item);
      }).then(() => {
        this.loading = false;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除关于我们编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delAbout(ids);
      }).then(() => {
        this.loading = false;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lover/about/export', {
        ...this.queryParams
