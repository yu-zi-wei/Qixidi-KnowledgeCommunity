      ...
      <el-table-column label="照片" align="center" prop="img">
        <template slot-scope="scope">
          <el-image
            ...
            :preview-src-list="srcList" fit="contain"/>
          <span v-else>暂无图片</span>
        </template>
      </el-table-column>
      <el-table-column label="拍摄地址" align="center" prop="address"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="简介" prop="remarks">
          <el-input type="textarea" v-model="form.remarks" maxlength="200"
                    show-word-limit placeholder="请输入备注" rows="4"/>
        </el-form-item>
        <el-form-item label="拍摄地址" prop="address">
          <el-input v-model="form.address" maxlength="40" show-word-limit placeholder="请输入地址"/>
        </el-form-item>
...
<script>
import {listAlbum, getAlbum, delAlbum, addAlbum, updateAlbum} from "@/api/lover/album";
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAlbum(id).then(response => {
        ...
        this.value.push({url: this.form.img, ossId: this.form.imgId})
        this.open = true;
        this.title = "修改时光相册";
      });
    },
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.value.length > 0) {
            this.form.img = this.value[0].url;
            this.form.imgId = this.value[0].ossId;
          }
    ...
    updateState(item) {
      this.$modal.confirm('是否确认修改时光相册编号为"' + item.id + '"的数据项？').then(() => {
        this.loading = true;
        updateAlbum(item);
      }).then(() => {
        this.loading = false;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除时光相册编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delAlbum(ids);
      }).then(() => {
        this.loading = false;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lover/album/export', {
        ...this.queryParams
