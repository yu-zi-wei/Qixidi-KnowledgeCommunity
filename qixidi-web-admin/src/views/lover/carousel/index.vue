  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        ...
        <span class="color-error ml-8">
          ...
          提示：如果多个类型（同时启用），前台默认取类型靠前的前五条（更换类型时请关闭其他类型）
        </span>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="简介" prop="introduce">
          <el-input v-model="form.introduce" type="textarea" :rows="3" maxlength="40" show-word-limit
                    placeholder="请输入简介"/>
        </el-form-item>
...
<script>
import {listCarousel, getCarousel, delCarousel, addCarousel, updateCarousel} from "@/api/lover/carousel";
...
export default {
  ...
  methods: {
    ...
    updateState(item) {
      this.$modal.confirm('是否修改轮播图编号为"' + item.id + '"的数据项？').then(() => {
        this.loading = true;
        updateCarousel(item);
      }).then(() => {
        this.loading = false;
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除爱情轮播图编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('lover/carousel/export', {
        ...this.queryParams
