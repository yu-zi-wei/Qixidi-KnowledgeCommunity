...
**规则提炼**：选择性提炼——只记"写代码时要用到的规范与约定"（结论形态），不记过程叙事（rules 每次会话自动加载，控制体积）。

**值得提炼**：
- 开发约定：bean 注入方式、装配位置、复用的工具类/方法、配置惯例
- 框架/组件级的坑与行为差异（版本升级坑、配置绑定规则变化）
- 环境差异类问题、实证过的诊断方法论（压缩为结论）

**不提炼**：踩坑过程叙事、链路细节描述、事故复盘流水账——结论留下，过程不记。
