            ...
            <el-row>
              ...
              <el-col :span="8">
                ...
                <el-button :loading="registerLoading" type="primary" key="key" :disabled="disable"
                           @click="getVerifyCodes(1)">{{ getCode }}
                </el-button>
              </el-col>
            </el-row>
          </el-form-item>
          ...
          <el-form-item>
            <el-row>
              ...
              <el-col :span="8">
                <!--                密码重置-->
                <el-button :loading="registerLoading" type="primary" key="key" :disabled="disable"
                           @click="getVerifyCodes(2)">{{ getCode }}
                </el-button>
              </el-col>
...
export default {
  ...
  methods: {
    resetPasswords() {
      this.$refs['registerFrom'].validate((valid) => {
        if (valid) {
          ...
          let fromData = {
            phone: this.registerFrom.email == null ? null : this.$base64.encode(this.registerFrom.email),
    ...
    getVerifyCodes(type) {
      this.$refs["registerFrom"].validate((valid) => {
        if (valid) {
          // const pathonJy = /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/;//手机号校验
