<template>
  <div class="article-list-admin">
    ...
    <div v-if="!loading">
      <div>
        <div v-for="(item,index) in articleList" :key="item.id || index"
          ...
          <div>
            ...
            <div>
              <nuxt-link :to="`/article-details/`+$base64.encode(item.id)" rel="noopener" target="_blank">
                <h1 class="font-s-18 mb-10 line-height-28 cursor-pointer text-underline-hover"
...
export default {
  ...
  methods: {
        ...
        switch (this.type) {
          ...
          case 3:
            ...
            this.$API("/white/article/sort", this.$get(), this.queryParams).then(res => {
              ...
              this.total = res.data.total;
              // 为新增的项目触发动画
              this.triggerNewGroupItemAnimations(startIndex3, res.rows.length);
            }).finally(() => this.scrollLoading = true);
            break
          default:
            break
      ...
      switch (this.type) {
        case 1:
          this.$API("/white/article/follow/list", this.$get(), this.queryParams).then(res => {
            ...
            // 确保loading完成后再触发动画
            this.$nextTick(() => {
              this.triggerArticleGroupAnimations();
            });
          });
          break
        case 2:
    ...
    // 为新增的项目触发动画
    triggerNewGroupItemAnimations(startIndex, count) {
      this.animator.triggerNewItemsAnimation(startIndex, count, 'articleGroupItem');
    },
  },
  mounted() {
    // 初始化动画器
    this.animator = createAnimator(this, 'groupArticle');
    this.getArticleList();
