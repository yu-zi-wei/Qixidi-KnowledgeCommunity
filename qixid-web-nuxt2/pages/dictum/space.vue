<template>
  <div>
    <div class="module-main">
      <div class="dictum-title-cl flex-direction-row">
        <div>
          <el-button plain size="medium" round icon="el-icon-back" @click="$router.push('/')">首页</el-button>
          <el-button size="medium" round type="primary" @click="$router.push('/dictum')">阅读随笔</el-button>
        </div>
        <div class="">
          <nuxt-link to="/" title="首页">
            <svg t="1761705729814" class="icon-theme-636e72 icon-size-36 svg-translateY-11" viewBox="0 0 1024 1024"
                 version="1.1"
                 xmlns="http://www.w3.org/2000/svg" p-id="32118">
              <path
        ...
        <div>
          ...
          <el-dropdown size="medium">
            <div class="el-dropdown-link" title="随笔管理">
              <el-button plain type="primary" size="medium" circle icon="el-icon-s-operation"></el-button>
            </div>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item @click.native="routeUrl('/user_admin/dictum/album-manage')">
                专辑管理
              </el-dropdown-item>
              <el-dropdown-item @click.native="routeUrl('/user_admin/dictum/manage?state=all')">随笔管理
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </div>
        ...
        <div class="flex-4">
          <div class="albumList-box" v-if="!loading">
            <div class="flex-space-between">
              ...
              <div>
                <el-button type="text" @click="dialogVisible=true"><i class="el-icon-plus"></i>添加</el-button>
              </div>
            </div>
            <hr class="hr-item-2 mt-6"/>
            <div class="space-list">
              <div v-for="(item,index) in dictumAlbumListArr"
                   :class="{'dictum-list-cl':true,'dictum-background-color-acti':$base64.decode($route.query.code)==item.id}"
                   @click="routeAlbumList('/dictum/space/content-list',$route.query.uuid,item.id)" :key="index">
                <div class="img-div">
                  <el-image v-if="item.cover" class="img-cover" :src="item.cover" fit="cover"></el-image>
            ...
            <div v-if="dictumAlbumListArr.length==0" class="text-center">
              <svg t="1682476949715" class="icon" viewBox="0 0 1024 1024" version="1.1"
...
export default {
  ...
  data() {
    return {
      ...
      queryParams: {
        pageNum: 1,
        pageSize: 100,
        uid: this.$route.query.uuid == null ? null : this.$base64.decode(this.$route.query.uuid)
      },
      albumValue: [],
      albumInfoRules: {
        name: [
  ...
  methods: {
    ...
    routeAlbumList(url, uuid, albumId) {
      this.$router.push({
        path: url,
        query: {
          uuid: uuid,
          code: this.$base64.encode(albumId)
        }
      });
    },
    addAlbum() {
...
.dictum-list-cl:hover .img-cover {
  transform: scale(1.2);
}
