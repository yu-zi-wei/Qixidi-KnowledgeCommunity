  ...
  <div class="mt-30">
            ...
            <el-menu
              ...
              <div v-for="item of toolArray">
                <el-menu-item :index="'/tool?tool='+item.id" @click="toolClick(item.id)">
                  <template slot="title">
                    <h2 class="font-s-16 " v-text="item.toolName"></h2>
                  </template>
                </el-menu-item>
              </div>
            </el-menu>
    ...
    <div class="_module_explicit">
      <el-menu
        ...
        text-color="#000000"
        class="el-menu-demo">
        <el-menu-item v-for="(item,index) in toolArray" :index="'/tool?tool='+item.id" @click="toolClick(item.id)" :key="index">
          <template slot="title">
      ...
      <div style="width: 100%">
        <div class="mb-10" style="width: 90%;margin: auto">
          <el-input placeholder="关键字（Enter 检索）" suffix-icon="el-input__icon el-icon-search"
                    v-model="query.toolName" @keyup.enter.native="isSearch = true"></el-input>
        </div>
        <div>
