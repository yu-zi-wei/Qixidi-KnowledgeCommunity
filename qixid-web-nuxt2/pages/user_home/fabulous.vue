<template>
  <div class="mb-10">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="点赞的文章" name="fabulousArticle">
        <div style="min-height: 200px;">
          ...
          <ul v-if="fabulousArr.length>0 && !loginLoading" class="fabulousArr-ul" ref="dialogContent"
            ...
            <li class="fabulousArr-li" v-for="(item,index) in fabulousArr" :key="index"
                :ref="`userHomeFabulousUserItem${index}`">
              <div class="flex-space-between align-items" style="padding-right: 10px">
                <div>
                  <div class="mb-15 font-s-14 color-grey">
                    <nuxt-link class="cursor-pointer hover-cl"
                               :to="'/user_home/article?uuid='+$base64.encode(item.userId)" target="_blank">
                      {{ item.nickname }}
                    </nuxt-link>
                    |
                    <span class="mr-4">{{ item.occupation == null ? '职业-~-' : item.occupation }}</span>|
                    <span>{{ $utils.parseTime(item.createTime, '{y}-{m}-{d}') }}</span>
                  </div>
                  <div class="mb-15">
                    <nuxt-link :to="`/article-details/`+$base64.encode(item.id)" target="_blank" rel="noopener">
                      <h1 class="font-s-18 mb-8 line-height-28 text-underline-hover">
                        {{ item.articleTitle}}</h1>
                    </nuxt-link>
                  </div>
                </div>
                <div>
                  <el-image class="_module_hiding" v-if="item.articleCover" :src="item.articleCover" fit="cover"
                            style="width: 120px;height: 80px;border-radius: 2px"></el-image>
                </div>
              </div>
          ...
          <div v-if="fabulousArr.length==0 && !loginLoading"
            ...
            <div class="font-s-14 color-grey-2">暂无点赞</div>
          </div>
        </div>
      </el-tab-pane>
      <el-tab-pane label="点赞的评论" name="followComment">
        <div style="min-height: 200px;">
          <div class="text-center">
            <svg t="1685706589002" class="icon-theme-2" viewBox="0 0 1024 1024" version="1.1"
                 xmlns="http://www.w3.org/2000/svg"
                 p-id="5864" width="40" height="60">
              ...
              <path d="M512 638c-19.8 0-36-16.2-36-36V314c0-19.8 16.2-36 36-36s36 16.2 36 36v288c0 19.8-16.2 36-36 36z"
                    p-id="5867"></path>
              <path d="M512 710m-36 0a36 36 0 1 0 72 0 36 36 0 1 0-72 0Z" p-id="5868"></path>
            </svg>
            <div class="font-s-14 color-grey-2">功能待开发</div>
          </div>
        </div>
      </el-tab-pane>
    </el-tabs>
...
.fabulousArr-ul::-webkit-scrollbar {
  width: 6px;
  height: 6px;
...
.fabulousArr-ul::-webkit-scrollbar-thumb {
  background: #ced6e0;
  border-radius: 2px;
}
</style>
