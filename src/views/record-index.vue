          ...
          <div class="overflow-hidden mb-20 text-center flex-cl-index">
            <div v-if="info.address">
              <div>
                ...
                <span class="span-2-cl" v-text="info.address"></span>
              </div>
            </div>
            <div class="ml-10" style="height: 30px;min-width: 10px;" v-if="info.createTime">
            <span>
                  <svg t="1669798181975" class="icon svg-16-f" viewBox="0 0 1024 1024" version="1.1"
                       xmlns="http://www.w3.org/2000/svg" p-id="9622" width="16" height="16"><path
                      ...
                      fill="#ffa502" p-id="9626"></path></svg>
                </span>
              <span class="span-2-cl" v-text="parseTimes(info.createTime,'{y}-{m}-{d}')"></span>
            </div>
          </div>
...
<script>
import {listRecordInfo} from "@/api/lover";
...
export default {
  ...
  methods: {
    ...
    listRecordInfos() {
      ...
      listRecordInfo(id).then(res => {
        this.info = res.data;
      })
    }
  },
  mounted() {
    this.listRecordInfos();
  }
}
</script>
