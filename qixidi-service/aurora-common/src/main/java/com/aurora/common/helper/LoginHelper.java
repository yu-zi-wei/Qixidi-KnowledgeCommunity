...
public class LoginHelper {
    ...
    private static final Map<String, TripartiteUser> TRIPARTITE_USER_CACHE_Map = new ConcurrentHashMap<>();
    ...
    public static void tripartiteLoginByDevice(TripartiteUser tripartiteUser, DeviceType deviceType) {
        ...
        TRIPARTITE_USER_CACHE_Map.put(TRIPARTITE_USER_KEY + JOIN_CODE + tripartiteUser.getUuid(), tripartiteUser);
        StpUtil.login(tripartiteUser.getLoginId(), deviceType.getDevice());
        log.info("存入当前用户数据：{}", TRIPARTITE_USER_CACHE_Map.get(TRIPARTITE_USER_KEY + JOIN_CODE + tripartiteUser.getUuid()));
        //登录
