package com.aurora.framework.listener;
...
public class UserActionListener implements SaTokenListener {
    ...
    public void doLogin(String loginType, Object loginId, SaLoginModel loginModel) {
        ...
        if (userType == UserType.SYS_USER) {
        ...
        } else if (userType == UserType.TRIPARTITE_USER) {
            //第三方登录
            TripartiteUser tripartiteUser = LoginHelper.getTripartiteUserHelper();
            UserAgent userAgent = UserAgentUtil.parse(ServletUtils.getRequest().getHeader("User-Agent"));
            String ip = ServletUtils.getClientIP();
            String tokenValue = StpUtil.getTokenValueByLoginId(loginId);
            UserOnlineDTO dto = new UserOnlineDTO();
            ...
            dto.setLoginTime(System.currentTimeMillis());
            dto.setTokenId(tokenValue);
            dto.setUserName(tripartiteUser.getUsername());
            RedisUtils.setCacheObject(Constants.ONLINE_TOKEN_KEY + tokenValue, dto, tokenConfig.getTimeout(), TimeUnit.SECONDS);
            log.info("user tripartiteLogin, userId:{}, token:{}", loginId, tokenValue);
